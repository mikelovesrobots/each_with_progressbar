...
#
# ConsoleProgressbar is a terminal-based progress bar library.
#
# == Usage
#
...
# == Later changes:
#
#   * Mike Judge added the more ruby-like extension to ConsoleProgressbar in 2007:
#
#   ConsoleProgressbar.new("my label", 1000) do |progressbar|
#     ids.each do |id|
#       progressbar.inc
#       # do work
#     end
...
class ConsoleProgressbar
  ...
  def show
    @last_update = Time.now - @start_time
  ...
  def show_progress
    ...
    if cur_percentage > prev_percentage || @is_finished || (@last_update + @update_every_secs).to_i <= time_running
      show
    end
  end
